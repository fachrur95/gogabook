...
import type {
  IMasterOther,
  MasterOtherType,
} from "@/types/masters/masterOther";
import debounce from "lodash.debounce";

const useInfiniteOther = ({ type }: { type: MasterOtherType }) => {
  ...
  useEffect(() => {
    if (data) {
      const dataOptions: IDataOption[] = data?.pages
        .map((page) =>
          page.result.map((row: IMasterOther) => ({
            id: row.id,
            label: row.masterother_description,
          }))
        )
        .flat();
...
export default useInfiniteOther;
