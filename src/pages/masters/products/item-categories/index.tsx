...
const sortDefault: GridSortModel = [
  { field: "masteritemcategory_description", sort: "asc" },
];

const title = "Master Item Category";
const path: WorkerPathType = "itemsitemcategory";
...
const MasterItemCategoryPage: MyPage<{ sessionData: ISessionData }> = ({
  sessionData,
}) => {
  const { data: menuRoles } = useMenuRole();
  const [rows, setRows] = useState<IMasterItemCategory[]>([]);
  ...
  const columns: GridColDef[] = [
    {
      ...
      flex: 1,
      renderCell: (
        params: GridRenderCellParams<unknown, IMasterItemCategory, unknown>
      ) => {
        const display = params.row.masteritemcategory_description;
    ...
    {
      field: "masteritemtype_description",
      headerName: "Type",
      type: "string",
      flex: 1,
      valueGetter: (
        params: GridValueGetterParams<unknown, IMasterItemCategory>
      ) => {
        return params.row.masteritemtype?.masteritemtype_description ?? "-";
      },
    },
    {
      field: "masteritemcategory_active",
      headerName: "Aktif",
      type: "boolean",
      filterable: false,
      flex: 1,
      renderCell: (
        ...
        <Chip
          icon={params.row.masteritemcategory_active ? <Done /> : <Close />}
          label={params.row.masteritemcategory_active ? "Aktif" : "Non-Aktif"}
          color={params.row.masteritemcategory_active ? "primary" : "secondary"}
        />
      ),
    },
    {
      field: "masteritemcategory_oleh",
      headerName: "Oleh",
      type: "string",
      flex: 1,
      hide: true,
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IMasterItemCategory[] = data?.pages
        .map((page) =>
          page.result.map((transaction: IMasterItemCategory) => transaction)
        )
        .flat();
...
MasterItemCategoryPage.Layout = "Dashboard";
