...
const sortDefault: GridSortModel = [
  { field: "masteritemtype_description", sort: "asc" },
];

const title = "Master Item Type";
const path: WorkerPathType = "itemtype";
...
const MasterItemTypePage: MyPage<{ sessionData: ISessionData }> = ({
  sessionData,
}) => {
  const { data: menuRoles } = useMenuRole();
  const [rows, setRows] = useState<IMasterItemType[]>([]);
  ...
  const columns: GridColDef[] = [
    {
      ...
      flex: 1,
      renderCell: (
        params: GridRenderCellParams<unknown, IMasterItemType, unknown>
      ) => {
        const display = params.row.masteritemtype_description;
    ...
    },
    {
      ...
      renderCell: (
        ...
        <Chip
          icon={params.row.masteritemtype_active ? <Done /> : <Close />}
          label={params.row.masteritemtype_active ? "Aktif" : "Non-Aktif"}
          color={params.row.masteritemtype_active ? "primary" : "secondary"}
        />
      ),
    },
    {
      field: "masteritemtype_oleh",
      headerName: "Oleh",
      type: "string",
      flex: 1,
      hide: true,
  ...
  useEffect(() => {
    if (data) {
      const dataRows: IMasterItemType[] = data?.pages
        .map((page) =>
          page.result.map((transaction: IMasterItemType) => transaction)
        )
        .flat();
...
export default MasterItemTypePage;
MasterItemTypePage.Layout = "Dashboard";
