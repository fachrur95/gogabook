...
const path: WorkerPathType = "all-trans";
...
const JournalEntriesPage: MyPage<{ sessionData: ISessionData }> = ({
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.allTransaction.getAll.useInfiniteQuery(
    {
      limit: 150,
      q: search,
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (
        ...
        <Chip
          icon={params.row.trans_status === "C" ? <Close /> : <Check />}
          label={params.row.trans_status === "C" ? "Deleted" : "Available"}
          color={params.row.trans_status === "C" ? "error" : "success"}
        />
      ),
    },
    {
