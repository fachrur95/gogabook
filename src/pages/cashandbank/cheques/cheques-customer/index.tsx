...
import { chequeTabs } from "@/components/tabs/data";
import NavTabs from "@/components/tabs";

const sortDefault: GridSortModel = [{ field: "trans_entrydate", sort: "desc" }];

const title = "Cheque Customers";
const path: WorkerPathType = "receivable-payments";
...
const ChequeCustomersPage: MyPage<{ sessionData: ISessionData }> = ({
  sessionData,
}) => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = api.payment.getAll.useInfiniteQuery(
    {
      type: path,
      limit: 150,
      q: search,
  ...
  useEffect(() => {
    setDataFilter((prev) => ({ ...prev, sortModel, filterModel }));
...
export default ChequeCustomersPage;
ChequeCustomersPage.Layout = "Dashboard";
