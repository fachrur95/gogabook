...
import type { IMasterPrefix } from "@/types/masters/masterPrefix";
// import { getServerSession } from "next-auth";

export const defaultUndefinedResult: InfiniteQueryResult<IMasterPrefix> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterPrefixRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IMasterPrefix>>(
      `${env.BACKEND_URL}/api/core/prefix?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
