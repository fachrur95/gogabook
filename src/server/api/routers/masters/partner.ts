...
export const defaultUndefinedResult: InfiniteQueryResult<IMasterPartner> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterPartnerRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IMasterPartner>>(
      `${env.BACKEND_URL}/api/core/partners?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
