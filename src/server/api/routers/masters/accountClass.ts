...
import type { IMasterAccountClass } from "@/types/masters/masterChartOfAccount/masterAccountClass";
// import { getServerSession } from "next-auth";

export const defaultUndefinedResult: InfiniteQueryResult<IMasterAccountClass> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterAccountClassRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IMasterAccountClass>>(
      `${env.BACKEND_URL}/api/core/accountclasses?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
