...
export const defaultUndefinedResult: InfiniteQueryResult<IMasterItemType> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterItemTypeRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IMasterItemType>>(
      `${env.BACKEND_URL}/api/core/itemtypes?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
