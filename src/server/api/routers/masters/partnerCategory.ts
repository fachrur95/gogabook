...
export const defaultUndefinedResult: InfiniteQueryResult<IMasterPartnerCategory> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterPartnerCategoryRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IMasterPartnerCategory>>(
      `${env.BACKEND_URL}/api/core/partnercategories?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
