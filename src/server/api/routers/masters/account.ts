...
export const defaultUndefinedResult: InfiniteQueryResult<IMasterAccount> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterAccountRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IMasterAccount>>(
      `${env.BACKEND_URL}/api/core/accounts?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
