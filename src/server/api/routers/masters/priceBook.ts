...
import type { IPriceBook } from "@/types/masters/priceBook";
// import { getServerSession } from "next-auth";

export const defaultUndefinedResult: InfiniteQueryResult<IPriceBook> = {
  result: [],
  count: 0,
  countAll: 0,
  currentPage: 0,
...
export const masterPriceBookRouter = createTRPCRouter({
  getAll: protectedProcedure.input(
    ...
    const result = await axios.get<InfiniteQueryResult<IPriceBook>>(
      `${env.BACKEND_URL}/api/core/prices?page=${cursor ?? 0}&size=${limit}${q ? `&q=${q}` : ""}${filter ? `&filter=${filter}` : ""}`,
